...
eval(function(h, b, i, d, g, f) {
    ...
    a.ias.defaults = {
        ...
        loaderDelay: 600,
        triggerPageThreshold: 2,//自动加载的数量 设置一次显示2页，即滑动2次出现下一页按钮
...
function() {
    ...
    a(document).ready(function() {
        ...
        a("#commentform").submit(function() {
            ...
            a.ajax({
                url: _deel.url+"submitComment",//提交评论
                ...
                success: function(B) {
                    ...
                    var objJson = JSON.parse(B);//将后台传过来的字符串转为json对象
                    var curComment= objJson.currentComment;//获取当前评论内容
                    //准备拼接当前评论
                    // https://cuiqingcai.com/avatar/.png
                    var currentComment='<li class="" id="comment-">' +
                        '<div class="c-avatar">' +
                        '<img alt="" src="'+_deel.url+objJson.user.user_headimage+'" class="avatar avatar-54 photo avatar-default" height="54" width="54" style="border-radius:50%;">' +
                        '<div class="c-main" id="div-comment-">' +
                        '<p>';
                    curComment = curComment.replace(/\s*/g,"");
                    var str = curComment.split(":");
                    for(var i=0;i<str.length;i++){
                        if(str[i]!=""){
                            if(g.smilies.hasOwnProperty(str[i])){
                                //这里是在判断这个smilies对象中是否有当前属性（key）
                               currentComment+='<img data-simle="' + str[i] + '" data-type="comment-smilie" src="'+_deel.url+'static/images/smilies/icon_' + g.smilies[str[i]] + '.gif">';
                            }else {
                                currentComment+=str[i];
                            }
                        }
                    }
                    currentComment+='</p>' +
                        '<div class="c-meta">' +
                        '<span class="c-author">'+objJson.user.user_name+'</span>' +
                        '</div>' +
                        '</div>' +
                        '<span id="success_1"></span>' +
                        '</li>';
                    B=currentComment;
                  //当前评论拼接完毕
        ...
        addComment = {
            moveForm: function(z, A, E, y, S, B) {
                //第一个参数z，主要是控制form表单所需的id=div-comment-
                //第二个参数A，是被回复人的id
                //第四个参数S，是主评论id
                var I = this,//this指代addComment
                w, D = I.I(z),//addComment调用了I方法返回div-comment-${vs.count}
                x = I.I(E),//返回respond
